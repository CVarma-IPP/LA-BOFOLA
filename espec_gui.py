...
class NewFileHandler(FileSystemEventHandler):
    ...
    def on_created(self, event):
        ...
        for _ in range(5):
            ...
            gui.log.append(f"[WARN] Cannot read file (permission): {filename}")
            return
...
class PlotCanvas(FigureCanvas):
    ...
    def update_plots(self,
        ...
        if (masked_energy is not None and
            ...
            exp_max = masked_intensity.max() if masked_intensity.size else 1.0
            tar_max = raw_target.max() if raw_target.size else 1.0
